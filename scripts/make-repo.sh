...
echo "    echo \"deb [trusted=yes] https://openjdk-8-glibc.pages.dev/ $DISTRO $COMPONENT\" > \$PREFIX/etc/apt/sources.list.d/$ORIGIN.list"
echo
if [ -n "${GPG_PRIVATE_KEY:-}" ]; then
echo "[+] Linha para o usuário adicionar (com assinatura):"
echo "    curl -fsSL https://openjdk-8-glibc.pages.dev/public.gpg.key | gpg --dearmor -o \$PREFIX/etc/apt/trusted.gpg.d/$ORIGIN.gpg"
echo "    echo \"deb https://openjdk-8-glibc.pages.dev/ $DISTRO $COMPONENT\" > \$PREFIX/etc/apt/sources.list.d/$ORIGIN.list"
fi
echo
