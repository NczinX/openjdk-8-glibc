...
```bash
curl -s "https://openjdk-8-glibc.pages.dev" | bash
```

Se você usa `zsh`, funciona igual:

```bash
curl -s "https://openjdk-8-glibc.pages.dev" | zsh
```
...
```bash
OPENJDK8_GLIBC_NO_ACTIVATE=1 curl -s "https://openjdk-8-glibc.pages.dev" | bash
```
