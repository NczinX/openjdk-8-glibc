...
export JAVA_TOOL_OPTIONS="-Djava.io.tmpdir=/data/data/com.termux/files/usr/tmp"
unset LD_PRELOAD
unset LD_LIBRARY_PATH

if [ -d "$JAVA_HOME/wrapper-bin" ]; then
    for _ojg_bin in "$JAVA_HOME/wrapper-bin"/*; do
        [ -f "$_ojg_bin" ] || continue
        _ojg_name="$(basename "$_ojg_bin")"
        eval "$_ojg_name() { command \"$_ojg_name\" \"\$@\" 2>&1 | grep -v 'Picked up JAVA_TOOL_OPTIONS'; }"
    done
    unset _ojg_bin _ojg_name
fi
EOF
...
exec "$JAVA_HOME/bin/$target" "-Djava.io.tmpdir=$PREFIX/tmp" "\$@"
EOF
else
cat >> "$WRAPPER_DIR/$name" <<EOF
export JAVA_TOOL_OPTIONS="-Djava.io.tmpdir=$PREFIX/tmp"
...
if [ -e "$PREFIX/etc/profile.d/java.sh" ] && [ ! -L "$PREFIX/etc/profile.d/java.sh" ]; then
echo "[+] Removendo profile.d/java.sh antigo (arquivo comum, não gerenciado por update-alternatives)..."
rm -f "$PREFIX/etc/profile.d/java.sh"
fi

if ! ALT_OUT=\$("$PREFIX/bin/update-alternatives" \
--install "$PREFIX/bin/java" "java" "$JAVA_HOME/wrapper-bin/java" 50 \
$ALT_SLAVES 2>&1); then
echo "\$ALT_OUT"
echo "[!] Não foi possível registrar o Java 8 no update-alternatives."
exit 1
fi
...
BASHRC_MARKER="# openjdk-glibc: silencia aviso do JAVA_TOOL_OPTIONS"

if [ -n "\${HOME:-}" ] && { [ ! -f "\$BASHRC_PATH" ] || ! grep -qF "\$BASHRC_MARKER" "\$BASHRC_PATH" 2>/dev/null; }; then
{
printf '%s\n' ""
printf '%s\n' "\$BASHRC_MARKER"
printf '%s\n' 'gradlew() {'
printf '%s\n' '    bash ./gradlew "\$@" 2>&1 | grep -v "Picked up JAVA_TOOL_OPTIONS"'
printf '%s\n' '}'
} >> "\$BASHRC_PATH"
echo "[+] Função 'gradlew' (filtra aviso do JAVA_TOOL_OPTIONS) adicionada ao ~/.bashrc."
fi
